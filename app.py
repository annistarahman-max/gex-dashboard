...
st.plotly_chart(fig_gex, key="gex", config={"displayModeBar": "hover", "modeBarButtonsToRemove": ["lasso2d", "select2d"]})
...
with st.expander("📊 VEX & CEX", expanded=not mode_fokus):
    ...
    with col_v:
        ...
        st.plotly_chart(fig_vex, key="vex", config={"displayModeBar": "hover", "modeBarButtonsToRemove": ["lasso2d", "select2d"]})
    ...
    with col_c:
        ...
        st.plotly_chart(fig_cex, key="cex", config={"displayModeBar": "hover", "modeBarButtonsToRemove": ["lasso2d", "select2d"]})
...
with col_iv1:
    if len(_iv_today) >= 2 and "atm_iv_smooth" in _iv_today.columns:
        ...
        st.plotly_chart(fig_iv_line, key="iv_line", config={"displayModeBar": "hover", "modeBarButtonsToRemove": ["lasso2d", "select2d"]})
    else:
        st.markdown(
...
with col_iv2:
    ...
    if len(_skew_src) > 0 and len(_skew) > 1:
        ...
        st.plotly_chart(fig_skew, key="iv_skew", config={"displayModeBar": "hover", "modeBarButtonsToRemove": ["lasso2d", "select2d"]})
        if _skew_excluded > 0:
            st.markdown(
...
with st.expander("🔥 Heatmap & Absolute Gamma", expanded=not mode_fokus):
    ...
    st.plotly_chart(fig_abs, key="abs_gex", config={"displayModeBar": "hover", "modeBarButtonsToRemove": ["lasso2d", "select2d"]})
    ...
    st.plotly_chart(fig_hm, key="heatmap", config={"displayModeBar": "hover", "modeBarButtonsToRemove": ["lasso2d", "select2d"]})
...
with st.expander("📈 Net Delta & OI", expanded=not mode_fokus):
    ...
    st.plotly_chart(fig_delta, key="delta", config={"displayModeBar": "hover", "modeBarButtonsToRemove": ["lasso2d", "select2d"]})
    if kpi.net_delta > 0:
